...
library(DT) # Allows display of data tables in html
library(shinyvalidate) #Shiny application
library(shinyBS) #Shiny application
library(zoo) # Shiny application
library(shinyjs) # Shiny application
...
library(openxlsx) # For data table export
library(flextable) # Data table creation
library(grid) # Shiny application
library(magick) # Shiny application
library(cowplot) # combines plots
...
# Only run if using specific site
...
# Example Here, primary input of harvest year switched from 2-3 Years,3-4 Years, 4 Years (August-October only)
...
# Ear hanging product sequence
################ Ear Hanging Upper Bound ########################

Output.Analysis.eh <- data.frame()
Productscen <- seq(200000,800000, by = 10000)
for(i in 1:length(Productscen))
{
  Product <- Productscen[i]
...
Output <- data.frame('Product' = Product, 'Harvest Year' = `Harvest Year`, 
                     ...
                     'Sea Days' = sum(Labor.metrics$Trips))

Output.Analysis.eh <- rbind(Output.Analysis.eh,Output)

}


################ Lantern Net product Upper Bound ########################

Output.Analysis.ln <- data.frame()
Productscen <- seq(100000,500000, by = 10000)
...
for(i in 1:length(Productscen))
{
  ...
  Output.Analysis.ln <- rbind(Output.Analysis.ln,Output)
  
}

# Labor totals
# Ear Hanging: 390,000 scallops
# Lantern Net: 230,000 scallops

####### Run gantt model outputs for labor only #################
...
Labor.tot <- rbind(Labor.LN, Labor.EH)

write.csv(Labor.tot, 'Labortot2.csv')


###### Create Chart ###################

Gantt <- read.csv('Labor_tot.csv')

Gantt$Category <- as.factor(Gantt$Category)
Gantt$Year <- as.factor(Gantt$Year)
Gantt$Start <- as.Date(Gantt$Start)
Gantt$End <- as.Date(Gantt$End)

year(Gantt$Start) <- 0
year(Gantt$End) <- 0

Gantt$Year <- factor(Gantt$Year, levels=c('Grow-Out Culture (2 Years)', 
                                          'Intermediate Culture','Seed Culture'))
Gantt$Category <- factor(Gantt$Category, levels=c('Seed Procurement', 'Restocking',
                                          'Cleaning/Gear Maintenance', 'Ear Hanging', 'Harvest'
                                          ))

ggplot(Gantt) +
  geom_linerange (aes (y=Year,
                       xmin = Start,
                       xmax = End,
                       color=Category ), size = 8) +
  theme_minimal()

library(scales)

lim <- as.Date(c("0000-01-01", "0000-12-31"))

Gantt$method <- as.factor(Gantt$method)

Gantt$Year <- as.factor(Gantt$Year)


 P1 <- ggplot(Gantt, aes(y = Year, xmin = Start,xmax = End, color = Category))  + 
  geom_linerange(aes(), size =5,position = position_dodge(width = .7))+
  facet_grid(Gantt$method, scales = "free") +
  theme_minimal() +
  ...
  ggtitle("Gantt Chart of Annual Labor Tasks and Estimated Timing") + 
  ylab("Year Class")+
  xlab("Month")+
  scale_x_date(
               date_breaks = "1 month", 
...
Gantt.eh <- subset(Gantt, Type != 'Lantern Net (Grow-Out)')
Gantt.eh$GO <- 'Ear Hanging'
Gantt.ln <- subset(Gantt, Type != 'Ear Hanging')
Gantt.ln$GO <- 'Lanter Net'

Gantt.go <- rbind(Gantt.eh, Gantt.ln)

P2 <- ggplot(Gantt, aes(x=X.1, y = method, fill = Category))  + 
  geom_bar(aes(),stat='identity',show.legend = FALSE)+
  theme_minimal() +
  theme(panel.grid.major.x=element_blank(), panel.grid.major.y=element_blank()) + #remove gridlines
  theme(panel.grid.minor.x=element_blank(), panel.grid.minor.y=element_blank()) + #remove gridlines
  ggtitle("Total Labor Days in an Annual Work Cycle") +
  ylab("Grow-Out Method")+
  xlab("Labor Days")+
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1))

Predicted.full$Harvest = paste(Predicted.full$Harvest.Y, Predicted.full$Harvest.Time, sep = "-")
Predicted.full$Harvest <- as.factor(Predicted.full$Harvest)

Predicted.full$Harvest <- factor(Predicted.full$Harvest, levels=c('Y1-Summer','Y2-Fall','Y2-Winter','Y2-Spring','Y2-Summer',
                                                                  'Y3-Fall','Y3-Winter','Y3-Spring','Y3-Summer'))

coeff <- max(Predicted.full$Sh_Height)/max(Predicted.full$Sh_Height.inches)

P3 <- ggplot(Predicted.full, aes(x=Harvest, y = Sh_Height.inches, color = Trial))  + 
  geom_point(aes(),show.legend = FALSE)+
  theme_minimal() +
  scale_y_continuous( limits = c(2, 5), breaks = seq(2,5,.5), sec.axis = sec_axis( trans=~.*coeff, name="Shell Height (mm)")
  ) +
  theme(panel.grid.major.x=element_blank(), panel.grid.major.y=element_blank()) + #remove gridlines
  theme(panel.grid.minor.x=element_blank(), panel.grid.minor.y=element_blank()) + #remove gridlines
  ylab("Shell Height (Inches)")+
  xlab("Harvest Timing")+
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1),
        axis.text.x = element_text(angle = 45, hjust = 1))

P4 <- ggplot(subset(Predicted.full, count.lbs < 30), aes(x=Harvest, y = count.lbs, color = Trial))  + 
  geom_point(aes())+
  theme_minimal() +
  theme(panel.grid.major.x=element_blank(), panel.grid.major.y=element_blank()) + #remove gridlines
  theme(panel.grid.minor.x=element_blank(), panel.grid.minor.y=element_blank()) + #remove gridlines
  ylab("Adductor Weight (Count/lbs)")+
  xlab("Harvest Timing")+
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1),
        axis.text.x = element_text(angle = 45, hjust = 1))
P5 <- plot_grid(P3,P4)

plot_grid(P1,P2,P5, ncol=1,labels = "AUTO")
