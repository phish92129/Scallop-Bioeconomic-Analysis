# Load wind data set from NERCOOS buoy F01 giving wind measurement in M/S starting in 2000 until 2024
# wind data is average wind and gusts
wind <- read.csv('F01_wind.csv')

# Remove time data and convert to date
wind$time = substr(wind$time, 1, 10)
wind$time <- as.Date(wind$time)
wind <- na.omit(wind)


# Average wind speed by day
wind.speed <- aggregate(data = wind, speed~time, FUN = 'max')
# Convert to knots
wind.speed$speed.knots <- wind.speed$speed*1.944
# Create a year column
wind.speed$Year = substr(wind.speed$time, 1, 4)
wind.speed$Year <- as.numeric(wind.speed$Year)

# Days where measurements occurred (to remove times where buoy was inoperative)
wind.speed.days.c <- table(wind.speed$Year)
wind.speed.days.c <- as.data.frame(wind.speed.days.c)

# load data frame of days in each year
year.days <- read.csv('yeardays.csv')

# merge columns 
wind.speed.days.c <- cbind(wind.speed.days.c,year.days)

# Create proportion factor to account for missing measurement days
wind.speed.days.c$Prop <- wind.speed.days.c$Tot.days/wind.speed.days.c$Freq

# Subset by winds above 25 knots i.e., weather days
wind.speed.sub <- subset(wind.speed, speed.knots >= 25)

# Count number of weather days in each year
wind.speed.sub.c <- table(wind.speed.sub$Year)
wind.speed.sub.c <- as.data.frame(wind.speed.sub.c)

# merge data sets
Wind.speed.days <- merge(wind.speed.days.c,wind.speed.sub.c,by='Var1')

# Create an adjusted rate of weather days based on non-operational buoy days rounded to the nearest weather day
Wind.speed.days$Adjusted.Freq <-(Wind.speed.days$Freq.y*Wind.speed.days$Prop) 

# Assuming that weather days are evenly distributed throughout the year, 
# subtract weekends for total available boat days

Wind.speed.days$Avail.workdays <- (Wind.speed.days$Tot.days)-104

# Calculate Total weather days and subtract from to

Wind.speed.days$Adjusted.weatherdays <- round(Wind.speed.days$Adjusted.Freq*(Wind.speed.days$Avail.workdays/Wind.speed.days$Tot.days))
Wind.speed.days$Adjusted.WorkDays <- Wind.speed.days$Avail.workdays-Wind.speed.days$Adjusted.weatherdays

# Subset out 2024 since the winter weather is skewing the data

Wind.speed.days <- subset(Wind.speed.days, Year != 2024)

Average.WorkDays <- round(mean(Wind.speed.days$Adjusted.WorkDays))
sd.WorkDays <- round(sd(Wind.speed.days$Adjusted.WorkDays))



# Create a unit for sea days
...
Output.Analysis.Punit <- data.frame()

Product.s <- seq(50000,500000, by = 10000)
HYear <- c('4 Years (August-October only)')
HSeason <- 'August-October'
GOMethod <- c('Ear Hanging', 'Lantern Net')
P.unit <- expand.grid(Product.s,HYear,HSeason,GOMethod)

for(i in 1:nrow(P.unit))
{
  
  Product <- P.unit[i,1]
  `Harvest Year` <- as.character(P.unit[i,2])
  `Harvest Season` <- as.character(P.unit[i,3])
  `Grow Out Method` <- as.character(P.unit[i,4])
  ...
  Output <- data.frame('Product' = Product, 'Harvest Year' = `Harvest Year`, 
                       ...
                       , 'Lease Footprint (Acres)' = Lease.Footprint$Acres, 'Sea Days' = sum(Labor.metrics$Trips))
  
  Output.Analysis.Punit <- rbind(Output.Analysis.Punit,Output)
  
}

write.csv(Output.Analysis.Punit, 'Output_Analysis_punit.csv')


  
  Product <- 390000
  `Harvest Year` <- '4 Years (August-October only)'
...
Labor.eh <- Labor.Subset
Labor.eh$Category[Labor.eh$Category == 'Seed procurement'] <- 'Seed Procurement'
Labor.eh$Category[Labor.eh$Category == 'Ear Hanging'] <- 'Ear-Hanging'

Labor.eh <- aggregate(Labor.eh, Trips~Category, FUN = 'sum')

o.land.d <- sum(Labor.metrics$Work.Days-Labor.metrics$Trips)
o.land <- c('On-Land Tasks', o.land.d)
Labor.eh <- rbind(Labor.eh,o.land)
Labor.eh$Go.Method <- 'Ear-Hanging'
Labor.eh$Trips <- as.numeric(Labor.eh$Trips)
Labor.eh$prop.trips <- (Labor.eh$Trips/sum(Labor.eh$Trips))
...
Labor.ln <- Labor.Subset
Labor.ln$Category[Labor.ln$Category == 'Seed procurement'] <- 'Seed Procurement'

Labor.ln <- aggregate(Labor.ln, Trips~Category, FUN = 'sum')

o.land.d <- sum(Labor.metrics$Work.Days-Labor.metrics$Trips)
o.land <- c('On-Land Tasks', o.land.d)
Labor.ln <- rbind(Labor.ln,o.land)
Labor.ln$Go.Method <- 'Lantern Net'
Labor.ln$Trips <- as.numeric(Labor.ln$Trips)
Labor.ln$prop.trips <- (Labor.ln$Trips/sum(Labor.ln$Trips))

Labor.ln$Production <- 220000
Labor.ln$min.scallop <- (Labor.ln$Trips/Labor.ln$Production)*3600

Labor.eh$Production <- 390000
Labor.eh$min.scallop <- (Labor.eh$Trips/Labor.eh$Production)*3600

Labor.prod <- rbind(Labor.eh,Labor.ln)


# Bar plot

Labor.prod$Category <- factor(Labor.prod$Category, levels=c('On-Land Tasks','Seed Procurement','Restocking',
                                                            'Harvest', 'Ear-Hanging','Cleaning'))


ggplot(Labor.prod, aes(x=Go.Method, y = prop.trips, fill = Category))  + 
  ...
  ylab("Proportion of Labor Days")+
  xlab("")+
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1))

ggplot(Labor.prod, aes(x=Go.Method, y = min.scallop, fill = Category))  + 
  geom_bar(aes(),position = "stack",stat='identity')+
  theme_minimal() +
  coord_flip() +
  theme(panel.grid.major.x=element_blank(), panel.grid.major.y=element_blank()) + #remove gridlines
  theme(panel.grid.minor.x=element_blank(), panel.grid.minor.y=element_blank()) + #remove gridlines
  ylab("Labor Seconds per Scallop")+
  xlab("")+
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1))
