...
# Entry and vectorization of primary parameters for business strategy
...
Y0 <- c('0-1 Years','all')
Y1 <- c('0-1 Years','1-2 Years','all')
Y2 <- c('0-1 Years','1-2 Years','2-3 Years','all')
Y3 <- c('0-1 Years','1-2 Years','2-3 Years','3-4 Years','all')
Y4 <- c('0-1 Years','1-2 Years','2-3 Years','3-4 Years','4 Years (August-October only)','all')

# Create vectors for seasonal harvest
Fall <- 'August-October'
...
Lease.Footprint$Feet.Longline.Total <- (Lease.Footprint$`Longline.Quantity`*`Individual Longline Length (ft)`) + 
  ((Lease.Footprint$`Longline.Quantity`*2) * sqrt(((`Longline Depth (ft)`-`Longline Suspended Depth (ft)`)*`Mooring Length`)^2 - (`Longline Depth (ft)`-`Longline Suspended Depth (ft)`)^2))
...
Lease.Footprint$A.Feet <- Lease.Footprint$Feet.Longline.Total * `Longline Spacing (ft)`
...
Lease.Footprint$Acres <- (Lease.Footprint$Feet.Longline.Total*`Longline Spacing (ft)`)*.0000229568
...
COG$Equipment <- ifelse(COG$Year == 0,sum(Equipment.Subset$Cost.Basis[which(Equipment.Subset$Year=='0-1 Years' | Equipment.Subset$Year=='all')]),
                        ifelse(COG$Year == 1,sum(Equipment.Subset$Cost.Basis[which(Equipment.Subset$Year== '1-2 Years')]),
                               ifelse(COG$Year == 2, sum(Equipment.Subset$Cost.Basis[which(Equipment.Subset$Year== '2-3 Years')]),
                                      ifelse(COG$Year == 3, sum(Equipment.Subset$Cost.Basis[which(Equipment.Subset$Year== '3-4 Years')]),0))))
...
Pane1 <- data.frame('Market Individuals' = round(Growth.Data$Market.Product),
                    'Shell Height (Inches)' = round(Growth.Data$Sh_Height.inches, digits=2),
                    'Adductor Count/lb' = round(Growth.Data$count.lbs),
                    'Run Rate (lbs)' = round(subset(COP, Year == 10)$`Run Rate (Adductor)`,digits=2), 
                    'Run Rate (Piece)' = round(subset(COP, Year == 10)$`Run Rate (Whole Scallop)`,digits=2),
                    '10 Year Break Even (lbs)' = round(subset(COP, Year == 10)$`Break Even Price (Adductor)`,digits=2),
                    '10 Year Break Even (Piece)' = round(subset(COP, Year == 10)$`Break Even Price (Whole Scallop)`,digits=2),
                    'Minimum Lease Size (Acres)' = round(Lease.Footprint$Acres,digits=2),
                    'Longline Quantity' = round(Lease.Footprint$`Longline.Quantity`))
...
plt.List <- list(LAB_plt = LAB_plt, COG_plt = COG_plt, FOC_plt = FOC_plt)
...
server <- function(input, output) {
  ...
  output$Y1_Product <- renderText({
    return(paste("Product: 1 Year - ", round(Mort.Calcs$Y1.Product, 0)))
  })
  
  output$Y2_Product <- renderText({
    return(paste("Product: 2 Years - ", round(Mort.Calcs$Y2.Product, 0)))
  })
  
  output$Y3_Product <- renderText({
    return(paste("Product: 3 Years - ", round(Mort.Calcs$Y3.Product, 0)))
  })
  
  output$Y4_Product <- renderText({
    return(paste("Product: 4 Years - ", round(Mort.Calcs$Y4.Product, 0)))
  })
  
  output$Ear.Hanging.Droppers <- renderText({
    return(paste("Total Dropper Lines: ", round(Mort.Calcs$Ear.Hanging.Droppers, 0)))
  })
  
  output$Dropper.Length <- renderText({
    return(paste("Dropper Length (ft): ", round(Mort.Calcs$Dropper.Length, 0)))
  })
  ...
  output$FOC <- renderPlot(plt.List$plt.List$FOC_plt)
  ...
  output$primary_inputs <- renderUI({
    ...
    box_list <- append(box_list, tagList(box(title = 'Large Initial Expenditures',
                                             width = 12,
                                             status = "info",
                                             solidHeader = TRUE,
  ...
  ###---------------------- Editable Table for major Cap-Ex purchases -----------------------------
  ...
  ###---------------------- Editable Table for Specialty Equipment ---------------------------
  ...
  observeEvent(input$run_model, {#Fake running of the model - updates all the thingy
    if (!iv$is_valid()) {
      ...
      plt.List$plt.List <- list(LAB_plt = LAB_plt, COG_plt = COG_plt, FOC_plt = FOC_plt, VarsTable = VarsTable)
