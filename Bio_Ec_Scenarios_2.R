...
S.Automated$Technology <- 'Automated Drilling Only'
S.Automated$Purchase.Method <- 'Private'
...
S.Dremmel$Purchase.Method <- 'Private'
...
S.Automated.coop$Purchase.Method <- 'Cooperative'
...
S.Dremmel.coop$Purchase.Method <- 'Cooperative'
...
S.DP.coop$Purchase.Method <- 'Cooperative'

# Remove FOC

Technology.Scenarios <- rbind(S.Automated,S.Automated.coop,S.Dremmel,S.Dremmel.coop,S.DP,S.DP.coop)

ggplot(subset(Technology.Scenarios,Year == 10), aes(x = Technology))  + 
  geom_bar(aes(fill = Purchase.Method, y = Debt/11), stat='identity', position=position_dodge())+
  theme_minimal() +
...
ggplot(Technology.Scenarios, aes(x = Technology))  + 
  geom_bar(aes(fill = Purchase.Method, y = MinPrice.AdductorWeight), stat='identity', position=position_dodge())+
  theme_minimal() +
  theme(panel.grid.major.x=element_blank(), panel.grid.major.y=element_blank()) + #remove gridlines
  theme(panel.grid.minor.x=element_blank(), panel.grid.minor.y=element_blank()) + #remove gridlines
  ylab("Run Rate (Price/lbs)")+
  labs(fill = "Purchasing Method")+
  xlab("Technology Choice")+
  theme(plot.title = element_blank (),
        strip.text.x = element_text(size=12, face="bold"), legend.position = "right",
        strip.text.y = element_text(size=12, face="bold"),axis.title.x = element_text(face="bold", size=12),
        legend.title = element_text(size=12, face="bold"), legend.text = element_text(size = 12),
...
ggplot(Technology.Scenarios, aes(x = Year))  + 
  geom_point(aes(color = Technology, y = MaxPrice.AdductorWeight, shape = Purchase.Method))+
  theme_minimal() +
  theme(panel.grid.major.x=element_blank(), panel.grid.major.y=element_blank()) + #remove gridlines
  theme(panel.grid.minor.x=element_blank(), panel.grid.minor.y=element_blank()) + #remove gridlines
  ylab("Annual Break Even")+
  labs(shape = "Purchasing Method", color = 'Technology Choice')+
  xlab("Year")+
  theme(plot.title = element_blank (),
        strip.text.x = element_text(size=12, face="bold"), legend.position = "right",
        strip.text.y = element_text(size=12, face="bold"),axis.title.x = element_text(face="bold", size=12),
